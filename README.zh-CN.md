...
- 不需要自建服务器的 GitHub Releases 更新检测
...
可选的更新检测会访问公开的 GitHub Releases API，用来判断是否有新版本。它不需要自建服务器、账号、遥测后端或托管更新服务。
...
PCTime 最多每天检查一次 GitHub Releases，也可以在设置页手动检查。发现新版本时，软件会显示最新版本，并给出 Windows 安装包下载入口。

这是为开源项目准备的轻量更新流程：不需要私有服务器，更新文件直接来自项目的 GitHub Release 页面。
